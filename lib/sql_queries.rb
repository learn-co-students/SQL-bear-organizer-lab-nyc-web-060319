...
def selects_most_prominent_color_and_returns_with_count
  str = 'SELECT color, COUNT(color) FROM bears GROUP BY color ORDER BY COUNT(color) DESC LIMIT 1'
  #binding.pry
  str
end
...
def selects_bear_that_killed_Tim
  'SELECT * FROM bears WHERE temperament is "aggressive" AND name is NULL'
end
